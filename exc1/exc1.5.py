...
def exc4():
    ...
    open('name_length.txt', 'w').write('\n'.join(str(len(name)) for name in open(file_path).read().split()))
def exc5():
    ...
    print('\n'.join(name for name in open(file_path).read().split() if len(name) == length_input))
